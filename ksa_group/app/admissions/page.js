...
export default function AdmissionsPage() {
  ...
  const eligibilityTable = [
    ...
    {
      course: "B.Pharm (4 Yrs)",
      college: "Kamarajar Pharmacy",
      criteria: "10+2 with Physics, Chemistry & Biology/Maths (Min 50%)",
      fees: "₹1,15,000 / Year"
    },
    {
      course: "Pharm.D (6 Yrs)",
      college: "Kamarajar Pharmacy",
      criteria: "10+2 with PCB/PCM or D.Pharm",
      fees: "₹1,90,000 / Year"
    },
    {
      course: "B.Sc Nautical Science (3 Yrs)",
      college: "Kamarajar Maritime",
      criteria: "10+2 with PCM (60%) & English (50%) + IMU CET + DGS Medical",
      fees: "₹2,25,000 / Year"
    },
    {
      course: "B.E. Marine Engg (4 Yrs)",
      college: "Kamarajar Maritime",
      criteria: "10+2 with PCM (60%) & English (50%) + IMU CET + DGS Medical",
      fees: "₹2,40,000 / Year"
    }
      ...
      <div className="bg-[#0A192F] text-white py-16 sm:py-20 text-center">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h1 className="font-heading font-extrabold text-4xl sm:text-5xl text-white mb-4">
            Join <span className="text-gradient-gold">KSA Group of Institutions</span>
          </h1>
          <p className="text-slate-300 text-base sm:text-lg max-w-2xl mx-auto">
            Clear eligibility criteria, merit-based scholarships, transparent fee structures, and immediate online application support.
          </p>

          <div className="mt-8">
            <button
              onClick={() => setModalOpen(true)}
              className="px-8 py-3.5 rounded-xl bg-gradient-to-r from-[#D4AF37] to-[#B8902A] text-[#0A192F] font-bold text-sm shadow-xl hover:brightness-110 cursor-pointer inline-flex items-center gap-2"
            >
      ...
      <section className="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#0A192F] text-[#D4AF37] text-xs uppercase font-bold tracking-wider">
            ...
            <tbody className="divide-y divide-slate-100 text-xs sm:text-sm text-slate-700">
              {eligibilityTable.map((row, idx) => (
                <tr key={idx} className="hover:bg-slate-50 transition-colors">
                  <td className="p-4 sm:p-5 font-bold text-[#0A192F]">{row.course}</td>
                  <td className="p-4 sm:p-5 text-slate-500 font-medium">{row.college}</td>
                  <td className="p-4 sm:p-5 text-slate-600">{row.criteria}</td>
                  <td className="p-4 sm:p-5 text-right font-extrabold text-[#0A192F]">{row.fees}</td>
                </tr>
              ))}
            </tbody>
          </table>
