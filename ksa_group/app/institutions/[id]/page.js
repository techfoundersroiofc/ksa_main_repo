...
import { ShieldCheck, MapPin, Phone, Mail, GraduationCap, CheckCircle2, ArrowRight } from "lucide-react";
    ...
    <div className="bg-white min-h-screen pb-20">
      ...
      <section className="relative bg-[#0A192F] text-white py-20 lg:py-28 overflow-hidden">
        <div className="absolute inset-0 z-0">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-r from-[#070F1E] via-[#0A192F]/90 to-[#0A192F]/70" />
        </div>

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="max-w-3xl space-y-4">
            <span className="px-3.5 py-1 rounded-full bg-[#D4AF37] text-[#0A192F] text-xs font-extrabold uppercase tracking-wider">
            ...
            <div className="pt-4 flex flex-wrap gap-4 text-xs text-slate-300">
              <span className="flex items-center gap-1.5 bg-slate-900/80 px-3 py-1.5 rounded-lg border border-slate-800">
                <ShieldCheck className="w-4 h-4 text-[#D4AF37]" /> {institution.affiliation}
              </span>
              <span className="flex items-center gap-1.5 bg-slate-900/80 px-3 py-1.5 rounded-lg border border-slate-800">
                <ShieldCheck className="w-4 h-4 text-[#D4AF37]" /> {institution.accreditation}
              </span>
            </div>
          </div>
        </div>
      ...
      {/* Stats Bar */}
      <section className="bg-[#070F1E] py-10 border-b border-[#D4AF37]/20 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {institution.stats.map((st, idx) => (
              <div key={idx} className="text-center p-4 rounded-xl bg-slate-900/50 border border-slate-800">
                <span className="block text-2xl sm:text-3xl font-extrabold text-[#D4AF37] font-heading">
                  {st.value}
                </span>
        ...
        <div className="p-8 sm:p-10 rounded-3xl bg-slate-50 border border-slate-200/80 shadow-lg grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
          <div className="md:col-span-4">
            <img
              ...
              className="w-full h-64 sm:h-72 object-cover rounded-2xl border-2 border-[#D4AF37]"
            />
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {institution.programs.map((prog) => (
              <div key={prog.id} className="p-6 rounded-2xl bg-white border border-slate-200 shadow-md space-y-3">
                ...
                <p className="text-xs text-slate-500 font-medium">Duration: {prog.duration}</p>
      ...
      <section className="py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="p-8 rounded-3xl bg-[#0A192F] text-white flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap gap-4 text-xs font-bold text-[#D4AF37]">
