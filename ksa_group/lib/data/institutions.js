export const INSTITUTIONS = [
  {
    id: "nursing",
    slug: "kamarajar-college-of-nursing",
    name: "KAMARAJAR COLLEGE OF NURSING",
    shortName: "KAMARAJAR COLLEGE  OF NURSING",
    category: "Nursing",
    tagline: "Shaping Compassionate Caregivers, One Student at a Time",
    established: 2021,
    affiliation: "The Tamil Nadu Dr. M.G.R. Medical University",
    accreditation:
      "Approved by Govt. of Tamil Nadu (G.O. Ms. No. 316), TNNMC & Indian Nursing Council (INC)",
    description:
      "Kamarajar College of Nursing is more than just a place to earn a degree it’s a place where dreams take shape, confidence is built, and futures are transformed. Located in the peaceful town of Chidambaram, Tamil Nadu, our college is a safe and supportive home for students who are passionate about helping others through the nursing profession.",
    heroImage: "/college/heroimage1.webp",
    campusImage:
      "/college/Building-the-Future-of-Nursing-with-Integrity-and-Purpose.webp",
    stats: [
      { label: "Established Under Trust", value: "2021" },
      { label: "Statutory Approvals", value: "INC & TNNMC" },
      { label: "University Affiliation", value: "TN M.G.R." },
      { label: "Placement Record", value: "100%" },
    ],
    about: {
      heading: "Shaping Compassionate Caregivers, One Student at a Time",
      paragraph1:
        "Kamarajar College of Nursing is more than just a place to earn a degree—it’s a place where dreams take shape, confidence is built, and futures are transformed. Located in the peaceful town of Chidambaram, Tamil Nadu, our college is a safe and supportive home for students who are passionate about helping others through the nursing profession.",
      paragraph2:
        "Initiated in 2021 under the KSA Educational and Charitable Trust, we offer high-quality nursing education blending academic knowledge with real-world experience and strong human values. Affiliated with The Tamil Nadu Dr. M.G.R. Medical University and approved by the Government of Tamil Nadu (G.O. Ms. No. 316), Tamil Nadu Nurses and Midwives Council (TNNMC), and the Indian Nursing Council (INC), our students receive personal attention in a disciplined, respectful environment. We focus on empowering rural and underprivileged women through education—giving them tools to build brighter futures for themselves and their communities.",
      keyBadges: [
        "Govt. of Tamil Nadu Approved",
        "INC & TNNMC Registered",
        "The TamilNadu M.G.R. University Affiliated",
        "100% Campus Placement Rate",
      ],
    },
    highlights: [
      "Affiliated with Tamil Nadu Dr. M.G.R. Medical University & Approved by INC, TNNMC & Govt. of TN",
      "Empowering Rural & Underprivileged Women Through Transformative Nursing Education",
      "Ragging-Free, Safe & Secure Campus with 24/7 CCTV Surveillance & Attendance Monitoring",
      "State-of-the-Art Skill Labs, E-Learning Infrastructure & Medlar/Medline Research Access",
    ],
    motto: "Arise and Shine, Radiate Brilliance",
    vision:
      "To emerge as a distinguished epicenter of excellence in nursing education, dedicated to sculpting consummate nursing professionals who epitomize the highest standards of compassionate care, unwavering ethics, and profound human values.",
    mission:
      "To transcend boundaries in achieving supremacy in delivering unparalleled, empathetic, and avant-garde nursing care. We are devoted to empowering the education of marginalized village women, fostering their ascent from socio-economic adversity into beacons of intellectual and professional brilliance.",
    recognitions: [
      {
        title: "Government of Tamil Nadu",
        subtitle: "G.O. Ms. No. 316",
        description:
          "Approved by the Government of Tamil Nadu under G.O. Ms. No. 316, operating in full accordance with state education policies and public health standards.",
      },
      {
        title: "Tamil Nadu Nurses and Midwives Council (TNNMC), Chennai",
        subtitle: "State Regulatory Authority",
        description:
          "Registered and approved by the Tamil Nadu Nurses and Midwives Council, which regulates nursing education and professional practice within the state.",
      },
      {
        title: "Indian Nursing Council (INC), New Delhi",
        description:
          "Recognized by the Indian Nursing Council, the apex national body for nursing education in India, ensuring national compliance and standard alignment.",
      },
      {
        title: "The Tamil Nadu Dr. M.G.R. Medical University, Chennai",
        description:
          "Affiliated with The Tamil Nadu Dr. M.G.R. Medical University. This affiliation guarantees that our syllabus, examinations, and degree certification adhere to rigorous academic standards.",
      },
    ],
    whyRecognitionsMatter: [
      {
        title: "Assurance of Quality",
        description:
          "You are enrolling in a program that is officially validated and structured by leading health and education authorities.",
      },
      {
        title: "National & Statewide Recognition",
        description:
          "Your nursing qualification will be recognized for licensure, employment, and higher studies across India.",
      },
      {
        title: "Global Relevance",
        description:
          "Our adherence to INC and state council standards enhances the global credibility of your nursing degree.",
      },
      {
        title: "Path to Licensing & Jobs",
        description:
          "Our recognitions support seamless registration with state nursing councils and eligibility for competitive placements.",
      },
    ],
    whyChooseKCON: [
      {
        title: "Ragging-Free Campus",
        description: "A harmonious and secure academic environment.",
      },
      {
        title: "Safe and Secured Premises",
        description: "A campus fortified with robust safety measures.",
      },
      {
        title: "Medlar/Medline Access",
        description: "Cutting-edge medical research resources.",
      },
      {
        title: "E-Learning Infrastructure",
        description: "Technology-driven, innovative learning platforms.",
      },
      {
        title: "State-of-the-Art Skill Laboratories",
        description: "Immersive practical learning environments.",
      },
      {
        title: "Advanced Training Programmes",
        description: "Comprehensive skill enhancement initiatives.",
      },
      {
        title: "Interstate Educational Visits",
        description: "Enriching exposure beyond regional boundaries.",
      },
      {
        title: "Outstanding Placement Records",
        description: "An impressive 100% campus placement rate.",
      },
      {
        title: "Clinical Excellence",
        description:
          "Collaborative practice with premier government organizations.",
      },
      {
        title: "24/7 CCTV Surveillance",
        description: "Uncompromised campus security at all hours.",
      },
      {
        title: "Attendance Monitoring System",
        description: "Precision-driven academic oversight.",
      },
      {
        title: "E-Governance Parent Interaction",
        description: "Seamless communication with guardians.",
      },
      {
        title: "Student Research Initiatives",
        description: "Encouragement of academic and scientific inquiry.",
      },
      {
        title: "Enrichment Endeavours",
        description:
          "Diverse extra-curricular opportunities for holistic development.",
      },
    ],
    committees: [
      "Admission Oversight Committee",
      "College Development Cell",
      "Anti-Ragging Enforcement Cell",
      "Student Grievance Redressal Cell",
      "Women’s Empowerment Cell",
      "Career Guidance and Placement Cell",
      "Research Advancement and Development Cell",
      "Sports and Athletic Cell",
      "Cultural Enrichment Committee",
      "Scholarly Journal Club",
      "Transportation Management Cell",
      "Hostel Management Cell",
      "Alumni Relations and Networking Cell",
    ],
    associations: [
      "Student Nurses Association (SNA)",
      "National Service Scheme (NSS)",
      "Youth Red Cross Society (YRC)",
      "Red Ribbon Club (RRC)",
      "Rotaract Club of Kamarajar College of Nursing",
    ],
    programs: [
      {
        id: "bsc-nursing",
        name: "B.Sc Nursing",
        level: "Undergraduate",
        duration: "4 Years",
        eligibility: "HSC / 10+2 with Physics, Chemistry, Biology & English",
        intake: 60,
        description:
          "A foundational healthcare degree producing skilled, empathetic, and professional nurses for clinical settings, hospitals, and community care globally.",
        highlights: [
          "INC & TNNMC Approved Syllabus",
          "Digital learning tools & skill labs",
          "Direct hospital clinical rotations",
        ],
      },
    ],
    scopeOfProgram: {
      title: "Career Scope & Global Demand for B.Sc Nursing Graduates",
      description:
        "Nursing is one of the most resilient, high-demand healthcare professions worldwide. Graduates of Kamarajar College of Nursing gain intensive clinical rotation experience that prepares them for immediate employment in government hospitals, private multispecialty chains, military nursing, and international healthcare systems (NHS UK, USA, Gulf countries).",
      roles: [
        "Industrial Nurse",
        "Nurse Practitioner",
        "Nursing Officer",
        "School Health Nurse",
        "ICU & Critical Care Specialist",
        "Nurse Educator / Clinical Instructor",
        "Healthcare Administrator",
        "Public Health & Community Specialist",
        "Global Travel Nurse",
      ],
      recruiters: [
        "Apollo Hospitals",
        "Fortis Healthcare",
        "MIOT International",
        "Kauvery Hospital",
        "SRMC",
        "Chettinadu Hospital",
        "NHS United Kingdom",
      ],
      averageSalary:
        "₹3.5 LPA - ₹8.5 LPA (India) | $45,000 - $85,000 / Year (Overseas)",
    },
    dean: {
      name: "Dr. S. Suganthra Devi, M.Sc(N), Ph.D(N).",
      title: "Message from the Principal",
      subtitle: "Nurturing Brilliance with Vision, Compassion, and Innovation",
      designation: "Principal, Kamarajar College of Nursing",
      qualification: "M.Sc Nursing, Ph.D in Nursing",
      experience: "24+ Years in Clinical & Academic Leadership",
      message:
        "Welcome to Kamarajar College of Nursing, a center of excellence where academic brilliance and compassionate care converge. Our mission is to shape future healthcare leaders by combining rigorous clinical training with deep empathy and ethical integrity.\n\nThrough state-of-the-art skill laboratories, innovative teaching methodologies, and continuous faculty development, we empower our students to excel in a dynamic global healthcare landscape.\n\nGuided by our motto 'Arise and Shine, Radiate Brilliance,' we nurture holistic growth, leadership, and clinical excellence—preparing caregivers to inspire a healthier tomorrow.",
      avatar: "/principal/Nursing.jpeg",
    },
    facilities: [
      {
        id: "nursing-sim",
        title: "High-Fidelity Clinical Simulation Ward",
        category: "Clinical Skill Lab",
        image: "/college/0M5A8536.JPG",
        description:
          "Computerized patient mannequins simulating cardiac arrests, vital sign fluctuations, pediatric emergencies, and maternal delivery for stress-free hands-on clinical mastery.",
        specs: [
          "Meti-Man Advanced Manikins",
          "ICU Telemetry Monitoring Systems",
          "Debriefing Video Control Rooms",
        ],
      },
      {
        id: "nursing-labs",
        title: "Anatomy, Physiology & Community Skill Labs",
        category: "Academic Labs",
        image: "/college/0M5A8628.JPG",
        description:
          "Comprehensive anatomical models, histological specimens, and community health kits ensuring thorough grounding in fundamental medical sciences.",
        specs: [
          "Interactive Human Body Models",
          "Microbiology Diagnostic Kits",
          "Community Outreach Field Kits",
        ],
      },
    ],
    contact: {
      phone: "+91-9488178596 (Principal), +91-9894064101 (CEO)",
      email: "ksaednltrust.nursing@gmail.com",
      location:
        "1069, Thirupaninatham, Keerapalayam, Chidambaram Taluk, Cuddalore Dt-608602, Tamil Nadu, India.",
      officeHours: "Mon - Sat: 8:30 AM - 5:00 PM",
    },
  },
  {
    id: "pharmacy",
    slug: "kamarajar-college-of-pharmacy",
    name: "Kamarajar College of Pharmacy",
    shortName: "KCP",
    category: "Pharmacy",
    tagline:
      "Pioneers in Pharmacy Education, Industrial Research & Clinical Care",
    established: 2017,
    affiliation: "The Tamil Nadu Dr. M.G.R. Medical University & PCI Approved",
    accreditation:
      "2nd Place Awardee in Upcoming Institution Award (TN M.G.R. University Research Day 2025) | 10 Industry & Hospital MoUs",
    description:
      "Kamarajar College of Pharmacy, Chidambaram, functioning under the KSA Educational and Charitable Trust, is committed to promoting quality education, professional excellence, research, innovation, and community development in the field of pharmacy and healthcare.",
    heroImage: "/college/Our-Nursing-Students-768x466.webp",
    campusImage: "/college/DJI_0542.webp",
    stats: [
      { label: "Dedicated Staff", value: "100+" },
      { label: "University Research Rank", value: "2nd Place" },
      { label: "Industry & Hospital MoUs", value: "10 MoUs" },
      { label: "Teaching Hospital Beds", value: "1,260 Beds" },
    ],
    about: {
      heading:
        "Promoting Professional Excellence, Research & Clinical Exposure",
      paragraph1:
        "Kamarajar College of Pharmacy, Chidambaram, functioning under the KSA Educational and Charitable Trust, is committed to promoting quality education, professional excellence, research, innovation, and community development in the field of pharmacy and healthcare. The Trust employs nearly 100 dedicated staff members and has a strong focus on promoting education in the rural sector while nurturing young talent for careers in pharmacy, pharmaceutical sciences, healthcare, research, and personalized medicine.",
      paragraph2:
        "The College is dedicated to the holistic development of students, giving importance to both their personal and professional growth. Through academic education, practical training, clinical exposure, research activities, outreach programmes, and industry and hospital collaborations, the institution strives to prepare students to meet the evolving needs of the healthcare sector.",
      keyBadges: [
        "PCI & TN M.G.R. Approved",
        "2nd Place Research Awardee 2025",
        "1,260-Bed Teaching Hospital MoU",
        "10+ Industrial & Hospital MoUs",
      ],
    },
    highlights: [
      "Conferred 2nd Place in Upcoming Institution Award among Pharmacy Colleges (TN M.G.R. University Research Day 2025)",
      "MoU with Govt Chidambaram Medical College Hospital (1,260-bed tertiary care teaching hospital) for Pharm.D Clinical Residency",
      "10 Memoranda of Understanding (MoUs) with pharmaceutical industries & hospitals for internships, R&D, and placements",
      "Specialized laboratories in Pharmaceutics, Pharmacology, Pharmaceutical Chemistry, Pharmacognosy, Analysis & Biotechnology",
    ],
    motto: "Innovation in Healing, Integrity in Practice",
    vision:
      "To become Pioneers in Pharmacy Education and Research by preparing skilled, ethical and compassionate pharmacists who can meet global healthcare needs.",
    mission:
      "To provide quality Pharmacy education with innovative learning and practices; develop competencies to meet changing professional needs; foster innovative research with industrial focus; and practice community-level pharmaceutical healthcare.",
    programs: [
      {
        id: "d-pharm",
        name: "Diploma in Pharmacy (D.Pharm)",
        level: "Diploma",
        duration: "2 Years",
        eligibility:
          "Candidates seeking admission to D.Pharm should have passed 10+2 examination in Science stream with Physics, Chemistry and Biology or Mathematics, or equivalent recognized under PCI & State Govt regulations.",
        intake: 60,
        description:
          "Introduced in 2017 with 60 approved seats. Provides a strong foundation in pharmaceutical sciences, medicine handling, patient interaction, dispensing, and hospital pharmacy practice.",
        highlights: [
          "PCI & TNMGRMU approved",
          "Practical drug dispensing labs",
          "Hospital pharmacy exposure",
        ],
      },
      {
        id: "b-pharm",
        name: "Bachelor of Pharmacy (B.Pharm)",
        level: "Undergraduate",
        duration: "4 Years",
        eligibility:
          "10+2 with English and Physics, Chemistry, Mathematics and/or Biology. Candidates with D.Pharm qualification from a PCI-approved institution are eligible for Lateral Entry to 3rd semester.",
        intake: 60,
        description:
          "Introduced in 2021 with 60 approved seats. Comprehensive education covering Pharmaceutics, Pharmacology, Pharmaceutical Chemistry, Pharmacognosy, Analysis, Pharmacy Practice, and Biotechnology.",
        highlights: [
          "10 Industry & Hospital MoUs",
          "2nd Place Awardee University Research Day 2025",
          "Dedicated R&D lab exposure",
        ],
      },
      {
        id: "pharm-d",
        name: "Doctor of Pharmacy (Pharm.D)",
        level: "Doctoral / Professional",
        duration: "6 Years (5 Yrs Academic + 1 Yr Hospital Internship)",
        eligibility:
          "1. Passed 10+2 with English and Physics, Chemistry, Mathematics (PCM) and/or Biology (PCB/PCMB) individually. 2. Minimum 50% aggregate in PCM/PCB. 3. Age: Completion of 17 years on or before 31st December of admission year.",
        intake: 30,
        description:
          "Introduced in 2025 with 30 approved seats. Clinical doctorate integrating hospital pharmacy, clinical toxicology, pharmacotherapeutics, and 1-year hands-on internship at Government Chidambaram Medical College Hospital (1,260 beds).",
        highlights: [
          "MoU with Govt Chidambaram Medical College Hospital (1,260 beds)",
          "1-Year full clinical residency internship",
          "Ward rounds & drug monitoring",
        ],
      },
      {
        id: "m-pharm-analysis",
        name: "M.Pharm - Pharmaceutical Analysis",
        level: "Postgraduate",
        duration: "2 Years",
        eligibility:
          "B.Pharm degree from an institution/university recognized by PCI and TNMGRMU satisfying PG regulations.",
        intake: 6,
        description:
          "Introduced in 2025 with 6 approved seats. Advanced PG program in pharmaceutical quality control, analytical research, laboratory instrumentation, and industrial QA/QC.",
        highlights: [
          "Advanced HPLC, FTIR & UV-Vis instrumentation",
          "Quality control & industrial lab focus",
          "Postgraduate thesis research",
        ],
      },
      {
        id: "m-pharm-ceutics",
        name: "M.Pharm - Pharmaceutics",
        level: "Postgraduate",
        duration: "2 Years",
        eligibility:
          "B.Pharm degree from an institution/university recognized by PCI and TNMGRMU satisfying PG regulations.",
        intake: 6,
        description:
          "Introduced in 2025 with 6 approved seats. Advanced education in pharmaceutical formulation development, novel drug delivery systems, pharmaceutical technology, and independent research.",
        highlights: [
          "Formulation technology R&D focus",
          "Industrial manufacturing prototype labs",
          "Independent thesis research",
        ],
      },
    ],
    scopeOfProgram: {
      title: "Career Scope & Industrial Opportunities for Pharmacy Scholars",
      description:
        "India is the global hub for pharmaceutical manufacturing and research. Graduates from KCP possess career mobility across pharmaceutical manufacturing, R&D, Quality Assurance/Quality Control, Clinical Research, Regulatory Affairs, Hospital Pharmacy, and Pharmacovigilance.",
      roles: [
        "Formulation R&D Scientist",
        "Quality Assurance / QC Manager",
        "Clinical Pharmacist & Therapeutics Expert",
        "Drug Inspector / Regulatory Officer",
        "Pharmacovigilance Specialist",
        "Hospital Pharmacy Director",
      ],
      recruiters: [
        "Sun Pharma",
        "Dr. Reddy's Laboratories",
        "Cipla",
        "Biocon",
        "Apollo Pharmacy",
        "Govt Chidambaram Medical College Hospital",
      ],
      averageSalary:
        "₹3.8 LPA - ₹9.0 LPA (Domestic) | ₹12+ LPA (R&D / Overseas)",
    },
    facultyList: [
      {
        id: 1,
        name: "Dr. G. Geetha",
        qualification: "M.Pharm., Ph.D",
        designation: "Principal",
      },
      {
        id: 2,
        name: "Mrs. R. Sudha",
        qualification: "M.Pharm., Ph.D",
        designation: "Vice Principal / Associate Professor",
      },
      {
        id: 3,
        name: "Dr. K. K. Sivakumar",
        qualification: "M.Pharm., Ph.D",
        designation: "Head of Department (Pharmaceutical Chemistry)",
      },
      {
        id: 4,
        name: "Dr. V. K. Kalaichelvan",
        qualification: "M.Pharm., Ph.D",
        designation: "Head of Department (Pharmacology)",
      },
      {
        id: 5,
        name: "Dr. A. Sriram",
        qualification: "M.Pharm., Ph.D",
        designation: "Head of Department (Pharmacy Practice)",
      },
      {
        id: 6,
        name: "Mrs. K. Arulselvi",
        qualification: "M.Pharm",
        designation: "Associate Professor",
      },
      {
        id: 7,
        name: "Mrs. V. G. Akilandeswari",
        qualification: "M.Pharm",
        designation: "Associate Professor",
      },
      {
        id: 8,
        name: "Mrs. A. Abinaya",
        qualification: "M.Pharm",
        designation: "Associate Professor",
      },
      {
        id: 9,
        name: "Mrs. B. Siva Ranjani",
        qualification: "M.Pharm",
        designation: "Associate Professor",
      },
      {
        id: 10,
        name: "Mr. T. Hariharan",
        qualification: "M.Pharm",
        designation: "Assistant Professor",
      },
      {
        id: 11,
        name: "Dr. R. Gomathi",
        qualification: "Pharm.D",
        designation: "Assistant Professor",
      },
      {
        id: 12,
        name: "Mr. A. Bharathi Raja",
        qualification: "M.Pharm",
        designation: "Assistant Professor",
      },
      {
        id: 13,
        name: "Dr. I. Shangeerthanasri",
        qualification: "Pharm.D",
        designation: "Assistant Professor",
      },
      {
        id: 14,
        name: "Mrs. S. Akalya",
        qualification: "M.Pharm",
        designation: "Assistant Professor",
      },
      {
        id: 15,
        name: "Mr. J. R. Chandra Prakash",
        qualification: "M.Pharm",
        designation: "Assistant Professor",
      },
      {
        id: 16,
        name: "Mrs. M. Saranya",
        qualification: "M.Pharm",
        designation: "Assistant Professor",
      },
      {
        id: 17,
        name: "Mrs. B. Nanthini",
        qualification: "M.Pharm",
        designation: "Assistant Professor",
      },
      {
        id: 18,
        name: "Mr. D. Palanivel",
        qualification: "B.Pharm",
        designation: "Lecturer",
      },
      {
        id: 19,
        name: "Mrs. P. Tamizhazhahi",
        qualification: "B.Pharm",
        designation: "Lecturer",
      },
      {
        id: 20,
        name: "Mrs. J. Hilma Dora",
        qualification: "B.Pharm",
        designation: "Lecturer",
      },
      {
        id: 21,
        name: "Dr. M. Krishnaveni",
        qualification: "M.Phil., Ph.D",
        designation: "Lecturer",
      },
      {
        id: 22,
        name: "Mrs. A. Sivaranjani",
        qualification: "M.Sc., M.Phil",
        designation: "Lecturer",
      },
      {
        id: 23,
        name: "Mrs. S. Maheswari",
        qualification: "B.Pharm",
        designation: "Lab Technician",
      },
      {
        id: 24,
        name: "Ms. R. Theivani",
        qualification: "D.Pharm",
        designation: "Lab Technician",
      },
      {
        id: 25,
        name: "Ms. M. Abina",
        qualification: "D.Pharm",
        designation: "Lab Technician",
      },
      {
        id: 26,
        name: "Ms. S. Kanimozhi",
        qualification: "D.Pharm",
        designation: "Lab Technician",
      },
      {
        id: 27,
        name: "Ms. P. Janani",
        qualification: "D.Pharm",
        designation: "Lab Technician",
      },
      {
        id: 28,
        name: "Ms. S. Mary George Pradhisha",
        qualification: "B.Pharm",
        designation: "Lab Technician",
      },
      {
        id: 29,
        name: "Mrs. T. Jayapriya",
        qualification: "MLIS",
        designation: "Librarian",
      },
      {
        id: 30,
        name: "Mrs. D. Akila",
        qualification: "M.Sc",
        designation: "Office Administration",
      },
      {
        id: 31,
        name: "Mrs. N. Anantha Lakshmi",
        qualification: "B.Sc",
        designation: "Office Administration",
      },
    ],
    dean: {
      name: "Dr. G. Geetha, M.Pharm., Ph.D.",
      title: "Principal & Head of Pharmacy Faculty",
      designation: "Principal (Vice Principal: Mrs. R. Sudha, M.Pharm., Ph.D.)",
      qualification: "M.Pharm, Ph.D in Pharmaceutical Sciences",
      experience: "20+ Years in Education & Industrial R&D",
      message:
        "The College is dedicated to the holistic development of students, combining academic excellence, practical competencies, and clinical exposure. With our 10+ MoUs and teaching hospital partnerships, we train future pioneers in pharmaceutical sciences and patient care.",
      avatar: "/principal/Pharmacy.jpeg",
    },
    facilities: [
      {
        id: "pharma-inst",
        title: "Central Instrumentation & Analysis Laboratory",
        category: "Research Facility",
        image: "/college/Our-Nursing-Students-768x466.webp",
        description:
          "Equipped with High-Performance Liquid Chromatography (HPLC), Fourier Transform Infrared Spectroscopy (FTIR), UV-Vis Spectrophotometer, and Rotary Evaporators for analytical research.",
        specs: [
          "HPLC & FTIR Instrumentation",
          "UV-Vis Spectrophotometers",
          "Analytical Balance & pH Meters",
        ],
      },
      {
        id: "pharma-form",
        title: "Pharmaceutics & Industrial Prototype Laboratory",
        category: "Manufacturing Lab",
        image: "/college/0M5A8628.JPG",
        description:
          "Tablet compression machines, dissolution test apparatus, disintegration testers, and liquid filling lines simulating industrial medicine manufacturing.",
        specs: [
          "Single-Punch Tablet Press",
          "8-Station Dissolution Tester",
          "Ointment & Capsule Filling Bay",
        ],
      },
    ],
    contact: {
      phone: "+91-9486419779 (Principal) , +91-9894064101 (CEO)",
      email: "ksaednltrust.pharma@gmail.com",
      location:
        "1069, Thirupaninatham, Keerapalayam, Chidambaram Taluk, Cuddalore Dt-608602, Tamil Nadu, India.",
      officeHours: "Mon - Sat: 8:30 AM - 5:00 PM",
    },
  },
  {
    id: "polytechnic",
    slug: "kamarajar-polytechnic-college",
    name: "Kamarajar Polytechnic College",
    shortName: "KPC",
    category: "Polytechnic",
    tagline:
      "Engineering Practical Skills, Technical Mastery & High-Paying Careers",
    established: 2015,
    affiliation:
      "Directorate of Technical Education (DoTE), Govt. of Tamil Nadu",
    accreditation:
      "AICTE Approved | 100% MNC Placement Support with Free Higher Education Sponsorship",
    description:
      "Kamarajar Polytechnic College (KPC), established in 2015 in Keerapalayam, Chidambaram, delivers industry-aligned diploma engineering programs. KPC features advanced engineering laboratories, high-speed computer & internet centers, green campus environment with sports grounds, and dedicated hostel & canteen facilities. With an annual fee of ₹40,000 and generous scholarships up to ₹37,500 per year, KPC prepares diploma engineers for top MNC placements offering attractive salaries and sponsored higher education.",
    heroImage: "/college/collegeabout.png",
    campusImage: "/college/7.jpeg",
    stats: [
      { label: "Annual Course Fee", value: "₹40,000" },
      { label: "Annual Scholarship", value: "₹37,500" },
      { label: "Net Yearly Fee", value: "₹2,500" },
      { label: "Placement & Higher Ed", value: "100% Support" },
    ],
    about: {
      heading: "Mastering Engineering Practice & Industrial Technology",
      paragraph1:
        "Kamarajar Polytechnic College delivers career-focused technical diploma education across Electrical & Electronics Engineering (EEE), Mechanical Engineering (MECH), and Automobile Engineering (DAE).",
      paragraph2:
        "Approved by AICTE and affiliated with DoTE Tamil Nadu, KPC provides hands-on practical workshops, high-speed internet computer centers, sports facilities, and comfortable hostel and canteen amenities. Every student benefits from our annual scholarship scheme of ₹37,500 against the ₹40,000 annual fee. Our placement wing ensures direct recruitment in top MNCs with attractive salary packages and options for free higher education.",
      keyBadges: [
        "AICTE Approved & DoTE Affiliated",
        "₹37,500 Annual Scholarship (Net Fee ₹2,500)",
        "Advanced Engineering & Computer Labs",
        "MNC Placement with Free Higher Education",
      ],
    },
    highlights: [
      "Advanced Engineering Laboratories & Practical Workshops for all streams",
      "High-speed Computer Center with Campus-Wide Internet Infrastructure",
      "Generous Annual Scholarship of ₹37,500 against ₹40,000 Course Fee",
      "Top MNC Placement Drives offering Attractive Salaries and Free Higher Education (B.E. Lateral Entry Sponsorship)",
      "Green Campus Environment, Playgrounds, Safe Hostel & Hygienic Canteen Infrastructure",
    ],
    motto: "Technical Mastery, Engineering Progress",
    vision:
      "To build a center of technical education excellence that transforms young minds into highly skilled, employable, and ethically sound diploma engineers.",
    mission:
      "To deliver hands-on technical instruction, modern laboratory training, and industrial exposure to produce industry-ready engineering technicians capable of leading modern industrial automation.",
    programs: [
      {
        id: "diploma-eee",
        name: "Diploma in Electrical & Electronics Engineering (EEE)",
        level: "Diploma",
        duration: "3 Years",
        eligibility: "10th Standard Pass (SSLC) or equivalent",
        intake: 60,
        description:
          "Comprehensive technical diploma covering electrical circuits, electronics, power systems, control machinery, and modern laboratory practicals.",
        highlights: [
          "Annual Fee: ₹40,000 | Scholarship: ₹37,500 (Net ₹2,500/Yr)",
          "Advanced Electrical & Power Automation Labs",
          "Top MNC Placements with Free Higher Education Sponsorship",
        ],
      },
      {
        id: "diploma-mech",
        name: "Diploma in Mechanical Engineering (MECH)",
        level: "Diploma",
        duration: "3 Years",
        eligibility: "10th Standard Pass (SSLC) or equivalent",
        intake: 60,
        description:
          "Practical diploma training in manufacturing technology, thermal engineering, CAD/CAM, fluid mechanics, and machine design.",
        highlights: [
          "Annual Fee: ₹40,000 | Scholarship: ₹37,500 (Net ₹2,500/Yr)",
          "Mechanical Workshops & High-speed Computer Center",
          "Top MNC Packages with Career & Degree Support",
        ],
      },
      {
        id: "diploma-auto",
        name: "Diploma in Automobile Engineering (DAE)",
        level: "Diploma",
        duration: "3 Years",
        eligibility: "10th Standard Pass (SSLC) or equivalent",
        intake: 60,
        description:
          "Specialized automotive diploma covering IC engines, vehicle dynamics, auto-electronics, servicing technology, and electric vehicles.",
        highlights: [
          "Annual Fee: ₹40,000 | Scholarship: ₹37,500 (Net ₹2,500/Yr)",
          "Automotive Engine & EV Diagnostic Labs",
          "Hostel & Canteen Facilities on Campus",
        ],
      },
    ],
    scopeOfProgram: {
      title: "Career Scope & Placement Pathways for Diploma Engineers",
      description:
        "Industrial automation, manufacturing, EV technology, and smart power grids have created immense demand for diploma engineers. KPC graduates gain direct recruitment into leading manufacturing MNCs, automobile giants, and energy enterprises, along with company-sponsored lateral entry into B.E./B.Tech programs.",
      roles: [
        "Industrial Automation Technician",
        "Junior Mechanical Design Engineer",
        "EV Service & Diagnostic Specialist",
        "Quality Control Inspector",
        "Plant Operations Supervisor",
        "Electrical Grid Technician",
      ],
      recruiters: [
        "Larsen & Toubro",
        "Hyundai Motors",
        "TVS Motors",
        "Tata Motors",
        "Bosch India",
        "Siemens",
      ],
      averageSalary:
        "Attractive Salary Packages + Free Higher Education (B.E./B.Tech) Sponsorship",
    },
    dean: {
      name: "Er. Vimalan L, M.Tech.",
      title: "Principal & Head of Engineering Faculty",
      designation: "Principal",
      qualification: "M.Tech",
      experience: "15+ Years in Technical Education & Industry",
      message:
        "At Kamarajar Polytechnic College, we emphasize practical application and industry readiness. Our dedicated faculty, high-tech labs, and generous scholarship schemes empower students to achieve success in top MNCs while continuing higher education.",
      avatar: "/principal/Poly.jpeg",
    },
    facultyList: [
      {
        sno: 1,
        name: "Er. Vimalan L",
        qualification: "M.Tech.",
        designation: "Principal",
      },
      {
        sno: 2,
        name: "Mr. Thanigainathan R",
        qualification: "M.Sc., M.Phil.",
        designation: "Lecturer",
      },
      {
        sno: 3,
        name: "Mr. Prabu E",
        qualification: "M.A., B.Ed.",
        designation: "Lecturer",
      },
      {
        sno: 4,
        name: "Mr. Senthil Kumar P",
        qualification: "B.E. (MECH)",
        designation: "Lecturer",
      },
      {
        sno: 5,
        name: "Mr. Raj Nirmal R",
        qualification: "B.E. (EEE)",
        designation: "HOD / EEE",
      },
      {
        sno: 6,
        name: "Mr. Veeraragavan K",
        qualification: "B.E. / B.Tech",
        designation: "HOD / Basic Engineering",
      },
      {
        sno: 7,
        name: "Mr. Nagarajan P",
        qualification: "MCA",
        designation: "Lecturer",
      },
      {
        sno: 8,
        name: "Mrs. Nithya M S",
        qualification: "M.E. CIVIL",
        designation: "HOD / DAE (Automobile)",
      },
      {
        sno: 9,
        name: "Mrs. Sharmila Banu N",
        qualification: "B.E. ECE",
        designation: "Lecturer",
      },
      {
        sno: 10,
        name: "Mr. Elayaraja P",
        qualification: "PET",
        designation: "Physical Education Trainer",
      },
    ],
    facilities: [
      {
        id: "mech-cnc",
        title: "Mechanical Lathe & CNC Machine Workshop",
        category: "Manufacturing Lab",
        image: "/college/collegeabout.png",
        description:
          "Heavy lathe machines, shaping machines, milling cutters, and CNC simulators where students learn precision machining and component fabrication.",
        specs: [
          "High-Precision Turning Lathes",
          "CNC Controller Simulation Bays",
          "Welding & Metallurgy Workshop",
        ],
      },
      {
        id: "auto-ev",
        title: "Automotive Engine & EV Diagnostic Lab",
        category: "Automotive Lab",
        image: "/faclities/6.JPG",
        description:
          "Cut-section engine models, wheel alignment rigs, automotive scanner tools, and Electric Vehicle (EV) battery testing infrastructure.",
        specs: [
          "Multi-Cylinder IC Engine Test Rigs",
          "EV Motor & Battery Diagnostics",
          "Automotive Scan Tool Systems",
        ],
      },
    ],
    contact: {
      phone: "8838930110 (Principal) / 9894064101 (CEO)",
      email: "ksaednltrust.poly@gmail.com",
      location:
        "1069, Thirupaninatham, Keerapalayam, Chidambaram Taluk, Cuddalore Dt-608602, Tamil Nadu, India.",
      officeHours: "Mon - Sat: 8:30 AM - 5:00 PM",
    },
  },
  {
    id: "maritime",
    slug: "kamarajar-college-of-maritime",
    name: "Perunthalaivar Kamarajar Institute of Maritime Science and Engineering",
    shortName: "PKIMSE",
    category: "Maritime",
    tagline:
      "Navigating Global Horizons with Elite Merchant Navy Training & Engineering",
    established: 2008,
    affiliation: "DGS grading",
    accreditation:
      "Change to Directorate General of Maritime Administration (DGMA) Approved",
    description:
      "Kamarajar College of Maritime is an elite merchant navy academy featuring a full-mission Ship Simulator, real engine workshop, swimming pool for sea survival, and a residential naval cadet lifestyle. Preparing officers and marine engineers for international shipping fleets.",
    heroImage: "/college/2.JPG",
    campusImage: "/faclities/4.jpg",
    stats: [
      { label: "Global Maritime Placement", value: "96%" },
      { label: "DGS Grade Rating", value: "A2 Outstanding" },
      { label: "Sailing Officer Alumni", value: "200+" },
    ],
    about: {
      heading: "Elite Nautical Academy & Merchant Navy Training Excellence",
      paragraph1:
        "Established in 2008, Kamarajar College of Maritime is an elite DGS-approved nautical academy dedicated to preparing deck ratings, engine ratings, and marine officers for international merchant shipping fleets.",
      paragraph2:
        "Featuring a DGS Grade 'A2' Outstanding rating, the academy offers residential naval cadet discipline, in-house heavy engine workshops, STCW safety & survival swimming pools, and merchant vessel placement support with a 96% global placement rate.",
      keyBadges: [
        "DGS Approved Academy",
        "Grade 'A2' Outstanding Rating",
        "96% Global Fleet Placement",
        "STCW Mandatory Safety Center",
      ],
    },
    highlights: [
      "In-House Engine Workshop & Heavy Marine Machining Lab",
      "STCW 2010 Mandatory Safety & Survival Training Center",
      "Strict Cadet Discipline & Officers' Mess Residential Life",
    ],
    motto: "Disciplined Sea Command, Global Leadership",
    vision:
      "To be a premier global center of maritime nautical education, instilling rock-solid seamanship discipline, modern navigation expertise, and marine engineering excellence in cadets.",
    mission:
      "To train disciplined deck and engine ratings for merchant fleets worldwide through hands-on seamanship workshops, STCW safety training, and rigorous naval residential cadet life.",
    programs: [
      {
        id: "gp-rating",
        name: "General Purpose (GP) Rating",
        level: "Certificate",
        duration: "6 Months",
        eligibility:
          "Class X pass from government recognized board with subjects English, Maths & Science with 40% aggregate. English 40% in X or XII (or) Pass in 2 Years ITI by DVET/NCVT with 40% & min 40% in English X or XII. Age: 17.5 to 28 on the date of commencement of course. Physical standard: Should be medically fit as per Merchant Shipping rules 2000",
        intake: 40,
        description:
          "Intensive practical seamanship training preparing deck and engine room ratings for merchant vessels.",
      },
      {
        id: "bst",
        name: "Basic STCW Safety Training Course (BST)",
        level: "Certificate",
        duration: "11 Days",
        eligibility:
          "10th Standard or equivalent + Medical Fitness Certificate",
        intake: 24,
        description:
          "Mandatory safety training program for all prospective seafarers, covering PST, FPFF, EFA, and PSSR in accordance with DGS and STCW guidelines.",
      },
      {
        id: "stsdsd",
        name: "Security Training for Seafarers with Designated Security Duties (STSDSD) 2 Days",
        level: "Certificate",
        duration: "2 Days",
        eligibility:
          "10th Standard or equivalent + Medical Fitness Certificate",
        intake: 24,
        description:
          "Mandatory 2-day DGS-approved STCW security training program for seafarers designated to perform security duties on merchant vessels.",
      },
    ],
    scopeOfProgram: {
      title: "Career Scope & Global Maritime Opportunities",
      description:
        "A career in international merchant shipping offers Tax-Free US Dollar salaries, rapid officer promotions, and global travel. GP Rating and STCW course graduates join international shipping companies as Deck Ratings, Engine Ratings, Trainee Officers, and Marine Mechanics.",
      roles: [
        "Merchant Navy Deck Rating",
        "Marine Engine Rating",
        "Able Seaman (AB) / Bosun",
        "Junior Marine Mechanic",
        "Offshore Vessel Operator",
        "Trainee Nautical Officer",
      ],
      recruiters: [
        "Maersk Line",
        "Fleet Management Limited",
        "Anglo-Eastern Ship Management",
        "Synergy Maritime",
        "MSC Shipmanagement",
        "V.Ships",
      ],
      averageSalary:
        "$ 200-500/ per month Trainee time $500-1000 per month after COP $800 -1000 per month Trainee officer",
    },
    dean: {
      name: "Ch.Engr -  Abdul Rasheed",
      title: "Principal of Maritime Studies",
      designation: "Principal & Engineer Superintendent",
      qualification: "Chief Engineer (DGS Class 1 Motor)",
      experience: "15+ Years at Sea & 4+ years Maritime Academy Leadership",
      message:
        "A career in maritime transportation offers unmatched adventure, leadership opportunities, and global financial rewards. We instill rock-solid nautical discipline and modern navigation skills in every cadet.",
      avatar: "/college/0M5A7948.JPG",
    },
    facilities: [
      {
        id: "ship-sim",
        title: "Full-Mission 360° Visual Ship Simulator",
        category: "Navigation Infrastructure",
        image: "/faclities/4.jpg",
        description:
          "DGS-approved Transas bridge simulator recreating realistic storm conditions, night navigation, and narrow channel passages for merchant navy cadets.",
        specs: [
          "360° Panoramic Visual Projection",
          "Dual Radar & ECDIS Consoles",
          "Global Sea Port Maps Loaded",
        ],
      },
      {
        id: "engine-workshop",
        title: "Heavy Marine Engine & Machinery Workshop",
        category: "Engineering Workshop",
        image: "/faclities/6.JPG",
        description:
          "Full-scale auxiliary marine engine components, lathe turning machines, electrical control panels, and welding bays for marine engine ratings.",
        specs: [
          "Operational Auxiliary Generator Unit",
          "Pneumatic & Hydraulic Test Benches",
          "Marine Safety & Firefighting Bay",
        ],
      },
    ],
    contact: {
      phone: "9894064101 (CEO / Admission Office)",
      email: "ksaednltrust@gmail.com",
      location:
        "1069, Thirupaninatham, Keerapalayam, Chidambaram Taluk, Cuddalore Dt-608602, Tamil Nadu, India.",
      officeHours: "Mon - Sat: 8:00 AM - 5:30 PM",
    },
  },
];

export const INSTITUTION_CATEGORIES = [
  "All Institutions",
  "Nursing",
  "Pharmacy",
  "Polytechnic",
  "Maritime",
];
